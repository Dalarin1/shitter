...
void Conn::cleanup_wsa() { WSACleanup(); }

void Conn::send_all(const std::string &_request) {
...
HttpConn::HttpConn(const std::wstring &host, INTERNET_PORT port) {
...
HttpConn::response HttpConn::get(const std::wstring &path) {
    if (hRequest) {
        WinHttpCloseHandle(hRequest);
        hRequest = nullptr;
    }
    ...
    if (WinHttpSetOption(hRequest, WINHTTP_OPTION_DECOMPRESSION, &decompression_flags,
        ...
        spdlog::warn("HttpConn: WinHttpSetOption decompression failed");
